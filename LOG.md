...
- **Ficheiro:** `20260813000002_ad_unlocks_fix.sql` — **aplicada manualmente pelo utilizador (2026-08-13)** ✅.
...
- **B13 — feedback de erro no unlock:** se o utilizador viu o anúncio mas `createUnlock` falhar (ex.: RPC ausente/rede), o `map.tsx:handleWatchAd` mostra `Alert` de erro em vez de falhar em silêncio.
- **B14 — limitação conhecida (documentada, sem fix por agora):** o RPC `create_ad_unlock` é executável por qualquer `authenticated` e **não prova que o anúncio foi visto** (honestidade do cliente) — um cliente malicioso pode chamá-lo directamente e gerar comissões sem ads. Fix real exigiria **AdMob Server-Side Verification (SSV)**.
...
| **`ad_unlocks`** | **0 rows** (nova) | RLS `relrowsecurity=true`; policies own mantidas apenas para **select/delete**; **INSERT revogado** (policy `ad_unlocks_insert_own` removida — criação/renovação só via RPC `create_ad_unlock`) e **UPDATE revogado** (policy `ad_unlocks_update_own` a remover via `20260813000003` — B11, fechava comissão sem anúncio); realtime na publication; trigger `trg_ad_commission` **AFTER INSERT OR UPDATE** com guarda anti-duplicação (`TG_OP='UPDATE' and new.expires_at <= old.expires_at` → return) e **mensagem dinâmica** com `to_char` + fallback 0.15 para `agent_commission_free_view_kz` inválido — verificado via `pg_get_triggerdef`/`pg_get_functiondef` |
